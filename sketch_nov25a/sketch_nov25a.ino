...
void loop()
{
  ...
  // 알고리즘 시작
  if(sensorValue[2] < sensorRef[2])  // 맨 오른쪽 센서(A0)에 라인이 감지된 경우
  {
    analogWrite(LMOTOR,180);
    analogWrite(RMOTOR,180);    
  }
  else if(sensorValue[3] < sensorRef[3])  // 맨 오른쪽 센서(A0)에 라인이 감지된 경우
  {
    analogWrite(LMOTOR,180);
    analogWrite(RMOTOR,180);    
  }
